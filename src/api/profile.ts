...
export async function getKycDocument(documentId: number): Promise<KycDocument | null> {
  const payload = await apiJson<unknown>(`/profile/kyc-documents/${documentId}`);
  if (!payload || typeof payload !== 'object') {
    return null;
  }
  const record = payload as Record<string, unknown>;
  const nested =
    (record.kyc_document && typeof record.kyc_document === 'object' ? (record.kyc_document as Record<string, unknown>) : null) ||
    (record.document && typeof record.document === 'object' ? (record.document as Record<string, unknown>) : null) ||
  ...
  return {
    id,
    document_type: typeof nested.document_type === 'string' ? nested.document_type : undefined,
    original_filename: typeof nested.original_filename === 'string' ? nested.original_filename : undefined,
    ...
    uploaded_at: typeof nested.uploaded_at === 'string' ? nested.uploaded_at : undefined,
  };
}
