...
function wizardGuaranteeFile() {
  const fileInput = document.getElementById('wiz-guarantee-file') as HTMLInputElement | null;
  return fileInput?.files?.[0] ?? null;
}

function requestWithoutGuarantee(request: StoreCreditRequestPayload): StoreCreditRequestPayload {
  const { guarantee: _guarantee, ...body } = request;
  return body;
}

async function upsertGuarantee(requestId: number, guarantee?: GuaranteeInput, file?: File | null) {
  if (!guarantee?.guarantee_type || !(guarantee.declared_value > 0)) {
    return;
  }
  const existing = await listCreditRequestGuarantees(requestId).catch(() => []);
  const sameType = existing.find((item) => (item.guarantee_type || '').toUpperCase() === guarantee.guarantee_type.toUpperCase());
...
async function hasExistingGuaranteeFile(requestId: number, guarantee?: GuaranteeInput) {
...
async function upsertRequest(request: StoreCreditRequestPayload, draftId?: number, guaranteeFile?: File | null) {
...
export async function saveDraftLoanApplication() {
  ...
  if (!request.credit_type) {
    throw new Error('Choisissez un type de crédit compatible avant d’enregistrer le dossier.');
  }
...
export async function submitCompleteLoanApplication(input?: {
  ...
  if (!request.credit_type) {
    throw new Error('Choisissez un type de crédit compatible avant d’envoyer le dossier à l’agent.');
  }
  ...
  const draftId = Number(fieldValue('wiz-draft-id')) || undefined;
  const guaranteeFile = wizardGuaranteeFile();
  const existingGuaranteeFile = draftId ? await hasExistingGuaranteeFile(draftId, request.guarantee) : false;
  if (!guaranteeFile && !existingGuaranteeFile) {
    throw new Error('Joignez le justificatif de garantie avant d’envoyer le dossier à l’agent.');
  }
