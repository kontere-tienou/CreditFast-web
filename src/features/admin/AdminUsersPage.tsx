...
export function AdminUsersPage() {
  ...
  const [accountType, setAccountType] = useState<"internal" | "external">(
    "internal",
  );
  ...
  const loadClients = async () => {
    ...
    try {
      setClients(await listAgentClients());
    } catch (error) {
      toast.danger(
        isApiError(error)
          ? error.message
          : "Impossible de charger les clients externes.",
      );
    } finally {
  ...
  const clientRows = useMemo(
    () =>
      clients
        .map((client) => {
          const status = isInactiveUser(client.user?.status)
            ? "inactive"
            : "active";
          return {
            id: String(client.id),
            name: agentClientName(client),
            email: client.user?.email ?? "",
            phone: client.user?.phone ?? "",
            clientNumber: client.client_number ?? `#${client.id}`,
      ...
      run: async () => {
        ...
        try {
          await updateAdminUser(Number(selected.id), {
            ...
            role:
              isSelf || isClient || !isStaffRole(selected.role)
                ? undefined
                : selected.role,
  ...
  const onReset = (event: FormEvent) => {
    ...
    if (selected.id === session?.userId) {
      toast.warning(
        "Utilisez Paramètres pour changer votre propre mot de passe.",
      );
      return;
    }
  ...
  return (
    <Screen viewId="view-admin-users">
      <PageHeader
        title={
          accountType === "internal" ? "Comptes internes" : "Clients externes"
        }
        crumbs={[
          "Administration",
          accountType === "internal" ? "Staff et administrateurs" : "Clients",
        ]}
        actions={
          <>
            <Button
              ...
              onClick={() => {
                setAccountType(
                  accountType === "internal" ? "external" : "internal",
                );
                setStatusFilter("ALL");
              }}
            >
              ...
              ></i>{" "}
              {accountType === "internal"
                ? "Voir les clients externes"
                : "Voir les comptes internes"}
            </Button>
            {accountType === "internal" ? (
              <Button onClick={() => setWizardOpen(true)}>
                <i className="fas fa-user-plus"></i> Créer un compte interne
              </Button>
            ) : null}
          </>
        }
      ...
      {accountType === "internal" ? (
        <AppTable
          title={
            loading
              ? "Chargement des comptes internes…"
              : "Tous les comptes internes"
          }
          ...
          columns={[
            ...
            },
            {
              ...
              label: "Contact",
              allowsSorting: true,
              render: (item) => item.email || item.phone || "—",
            },
            {
              id: "role",
              label: "Rôle",
            ...
            },
            {
              ...
              render: (item) => (
                <span
                  className={`badge ${item.status === "inactive" ? "badge-warning" : "badge-approved"}`}
                >
        ...
        <AppTable
          title={
            clientsLoading
              ? "Chargement des clients externes…"
              : "Tous les clients externes"
          }
          badge={`${clientRows.length} comptes`}
          items={clientRows}
          columns={[
            {
              id: "name",
            ...
            },
            {
              ...
              render: (item) => (
                <span
                  className={`badge ${item.status === "inactive" ? "badge-warning" : "badge-approved"}`}
                >
                  {item.status === "inactive" ? "Désactivé" : "Actif"}
                </span>
              ),
            },
          ]}
