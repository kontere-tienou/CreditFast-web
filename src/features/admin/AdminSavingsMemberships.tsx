...
import { membershipStatusLabel, SAVINGS_CHANGED } from '@/features/savings/workflow';
...
export function AdminSavingsMemberships() {
  ...
  async function load() {
    ...
    try { setRows(await listPendingMemberships('ALL')); }
  ...
  async function review(event: FormEvent<HTMLFormElement>, row: Membership) {
    ...
    const decision = data.get('decision') === 'REJECTED' ? 'REJECTED' : data.get('decision') === 'CHANGES_REQUESTED' ? 'CHANGES_REQUESTED' : 'APPROVED';
    ...
    if (decision !== 'APPROVED' && !value('rejection_reason')) { setError('Indiquez le motif du refus ou les éléments à compléter.'); return; }
    setBusy(row.id); setError(''); setNotice('');
    try {
      await reviewMembership(row.id, { decision, ...(decision === 'APPROVED' ? { account_number: value('account_number'), risk_level: value('risk_level'), caisse_signature: value('caisse_signature') } : decision === 'REJECTED' ? { rejection_reason: value('rejection_reason') } : { correction_reason: value('rejection_reason') }) });
      await load();
      window.dispatchEvent(new Event(SAVINGS_CHANGED));
      setNotice(decision === 'APPROVED' ? 'Adhésion validée. Le compte épargne est actif.' : decision === 'REJECTED' ? 'Adhésion refusée. Le demandeur pourra corriger sa fiche.' : 'Demande de compléments enregistrée.');
  ...
  return <section className="savings-admin" aria-labelledby="admin-savings-title">
    ...
    <div className="savings-grid"><label>Statut<select className="form-control" value={filter} onChange={event => setFilter(event.target.value)}>{['ALL', 'PENDING', 'CHANGES_REQUESTED', 'APPROVED', 'REJECTED'].map(status => <option key={status} value={status}>{status === 'ALL' ? 'Tous les statuts' : membershipStatusLabel(status)}</option>)}</select></label><label>Rechercher un demandeur<input className="form-control" value={search} onChange={event => setSearch(event.target.value)} /></label></div>
    {loading && <p role="status">Chargement des adhésions…</p>}
    {error && <p role="alert" className="savings-error">{error}</p>}
    {notice && <p role="status">{notice}</p>}
    {!loading && !error && !rows.some(row => (filter === 'ALL' || row.status === filter) && row.applicant_name.toLocaleLowerCase().includes(search.toLocaleLowerCase())) && <p>Aucune adhésion pour ces critères.</p>}
    {rows.filter(row => (filter === 'ALL' || row.status === filter) && row.applicant_name.toLocaleLowerCase().includes(search.toLocaleLowerCase())).map(row => <details key={row.id}>
      <summary>{row.applicant_name} — {row.client_type === 'LEGAL_ENTITY' ? 'Personne morale' : 'Personne physique'} — {membershipStatusLabel(row.status)}</summary>
      ...
      <p>{row.reviewed_at ? `Décision du ${new Date(row.reviewed_at).toLocaleString('fr-FR')}` : ''} {row.reviewed_by || ''}</p>
      {(row.rejection_reason || row.correction_reason) && <p>{row.rejection_reason || row.correction_reason}</p>}
      {row.status === 'PENDING' && <form onSubmit={event => void review(event, row)}><fieldset disabled={busy !== null} className="savings-section"><legend>Décision de l’administrateur</legend><div className="savings-grid">
        <label>Décision<select className="form-control" name="decision"><option value="APPROVED">Valider et activer le compte</option><option value="CHANGES_REQUESTED">Demander des compléments</option><option value="REJECTED">Refuser l’adhésion</option></select></label>
        ...
        <label>Motif du refus / éléments à compléter<textarea className="form-control" name="rejection_reason" maxLength={2000} /></label>
