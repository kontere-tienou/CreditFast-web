...
4. Compte épargne actif et pièce présente : navigation libre dans l’espace client. Le formulaire de prêt s’ouvre uniquement sur une action explicite (« Faire une demande de prêt », reprise de brouillon ou parcours du simulateur). Les règles de soumission et de validation du dossier restent applicables. Le revenu mensuel, les dépenses et les mensualités de crédit en cours ne se saisissent pas : `GET /profile/account-check` les reprend du compte (profil financier pour le revenu et les dépenses, somme des échéances des prêts actifs pour les mensualités). La création et la modification d’un dossier recopient ces trois montants ; une valeur envoyée par le formulaire est ignorée. Un prêt accordé crédite le capital sur le compte épargne, opération de type `LOAN_DISBURSEMENT`, sens CREDIT, libellée « Épargne + ».
